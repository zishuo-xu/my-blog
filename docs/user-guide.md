...
后台「备份管理」页面支持手动触发备份和查看备份历史：

| 功能 | 说明 |
|------|------|
| 立即备份 | 手动触发一次全量备份，上传至阿里云 OSS |
| 备份统计 | 总次数、成功数、失败数、最近备份时间 |
| 备份历史 | 文件名、大小、触发方式（手动/定时）、状态、下载链接 |

**自动备份**：默认每 3 天自动执行一次备份，上传至 OSS `backups/` 目录，超过 30 天的旧备份自动清理。
...
| `BACKUP_INTERVAL_DAYS` | 自动备份间隔（天） | `3` |
| `BACKUP_RETENTION_DAYS` | 备份文件保留天数 | `30` |
| `BACKUP_OSS_PREFIX` | OSS 中备份文件的存储前缀 | `backups` |
